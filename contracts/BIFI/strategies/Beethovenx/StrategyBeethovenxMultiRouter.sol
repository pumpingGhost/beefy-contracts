...
contract StrategyBeethovenxMultiRouter is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (toNative > 1000000000000000000) {
            balancerSwap(nativeSwapPoolId, output, native, toNative);
        }
        
        toNative = IERC20(secondOutput).balanceOf(address(this));
        if (toNative > 0) {
            IUniswapRouterETH(spookyRouter).swapExactTokensForTokens(toNative, 0, secondOutputToNativeRoute, address(this), now);
        }
    ...
    function addLiquidity() internal {
        ...
        IUniswapRouterETH(spookyRouter).swapExactTokensForTokens(nativeBal, 0, nativeToInputRoute, address(this), now);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(secondOutput).safeApprove(spookyRouter, uint256(-1));
        if (secondOutput != native) {
            IERC20(native).safeApprove(spookyRouter, uint256(-1));
        }
