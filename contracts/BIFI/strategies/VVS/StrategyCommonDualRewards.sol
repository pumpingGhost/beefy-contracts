...
contract StrategyCommonDualRewards is StratFeeManager {
    ...
    function rewardsAvailable() public view returns (uint256, uint256) {
        ...
        (, uint256[] memory rewards) = abi.decode(result, (address[],uint256[]));
    ...
    function callReward() public view returns (uint256) {
        ...
        if (secondOutputBal > 0) {
            uint256[] memory amountOut = IUniswapRouterETH(unirouter).getAmountsOut(outputBal, secondOutputToNativeRoute);
            nativeOut += amountOut[amountOut.length -1];
        }
